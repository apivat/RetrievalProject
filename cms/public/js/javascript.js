function confirmDownload(id,value) 
{
  if(confirm("คุณต้องเปลี่ยนแปลงการดาวน์โหลดของผู้ใช้นี้หรือไม่?"))
  {           
    $.post("./users/download", { id: id, value: value },
        function(data) {
            alert('ระบบได้ทำการเปลี่ยนเรียบร้อยแล้ว');  
...
function confirmStatus(id,value) 
{
  if(confirm("คุณต้องเปลี่ยนสถานะผู้ใช้นี้หรือไม่?"))
  {           
    $.post("./users/status", { id: id, value: value },
        function(data) {
            alert('ระบบได้ทำการเปลี่ยนเรียบร้อยแล้ว');  
            window.location.reload();
        }
...
function confirmDelete(id,table) 
{
  ...
  {       
    url = "http://localhost/retrieval/cms/confirm/delete/"+id+"/"+table;
    
    dojo.xhrPost({
       url: url,
       headleAs: "text",
       load: function(){
            alert('ระบบได้ทำการลบเรียบร้อยแล้ว');  
            window.location.reload();
       }
    });
  }
...
function changeStatus(num)
{
 ...
 path = 'http://localhost/retrieval/cms/public/img/icon/';
