...
<table width="99%" border="0" cellspacing="0" cellpadding="0" style="margin-top:8px;background:#FFF;">
    <tr class="title">
        <td class="row-head" style="border-left:1px solid #CCC;" width="65%">&nbsp;ชื่อวารสาร</td>
    ...
    <?php foreach ($result as $value): ?>
        <tr class="over">
            <td class="row-body" style="border-left:1px solid #CCC;">&nbsp;<?= $value->journal_title ?></td>
            ...
            <td class="row-body" align="right" style="border-right:1px solid #CCC;">
                <a href="<?= base_url() . $this->router->class . '/edit/' . $value->journal_id ?>"><img src="<?= base_url() ?>public/img/icon/page_edit.png" title="แก้ไข" /></a>&nbsp;
                <a href="javascript:confirmDelete('<?= $value->journal_id ?>','<?= $table ?>')"><img src="<?= base_url() ?>public/img/icon/delete.png" title="ลบ" /></a>&nbsp;
            </td>
        </tr>
    <?php endforeach; ?>
</table>
